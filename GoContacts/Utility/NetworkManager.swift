...
public typealias NetworkCompletion = (_ ContactsList: [Contact]?,_ error: String?)->()

class NetworkManager {
    static let sharedInstance = NetworkManager()
    private init() { } //will make sure class is not initialised anywhere else
    
    public func GetData (urlPath: String, completion: @escaping NetworkCompletion) {
        ...
        let task = session.dataTask(with: url) { data, response, error in
            ...
            case .success :
                ...
                do {
                    ...
                    print(jsonData)
                    let jsonReponse = try JSONDecoder().decode([Contact].self, from: responseData)
